var vm = new Vue({
  ...
  methods: {
    ...
    // getPdf: function () {
    //   var vm = this;
    //   if (!this.isOk && !confirm("图片还未全部加载完毕，可能会导致图片丢失，确定要导出PDF吗？")) {
    //     return false;
    //   }
  ...
  computed: {
    isOtherEvent: function () {
      var event = "罪案,有人受伤,纠纷,失火,损坏/故障,火警报告,保安警报";
      var result = [];
      if (this.list.type) {
        var type = this.list.type.split(",");
        type.forEach((e, i) => {
  ...
  mounted: function () {
    ...
    axios({
      ...
      console.log(list)
      vm.list = list;
      vm.list.rate = list.status == 2 ? list.operation[list.operation.length - 1].star : '未评分';
      //处理审批流程
      var operationlist = [];
      var operation = list.operation;
      var details = res.data.data[0].details;
