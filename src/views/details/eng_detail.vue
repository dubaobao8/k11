<template>
  <div>
    ...
    <section class="info-list">
      ...
      <d-info-item class="info-item" title="附件" v-if="info_data.pro_file&&info_data.pro_file.length"/>
    ...
    <section class="info-list">
      ...
      <div :key="Date.now() * Math.random()" class="ml15 mr15" v-for="(follow ,followIndex) in info_data.pro_follow">
        <p class="form-detail-title mb10 fw-b" style="color: #444;font-size: 0.95rem;">跟进记录{{followIndex + 1}}</p>
        <d-info-item :value="follow.pro_user.username" class="info-item" title="维修员"/>
        <d-info-item :value="follow.pro_identity.type"
                     class="info-item"
                     title="维修班组"/>
    ...
    <section class="form-group maintain-record mt20" style="overflow:hidden" v-if="canIEdit">
      ...
      <transition-group name="follow" tag="div">
        <!-- 维修记录开始 -->
        <div :key="maintain.uid" class="form-second-detail" v-for="(maintain, index) in form_model.maintain_record">
          ...
          <v-picker :options="[{text:'已完成',value:'已完成'},{text:'待料',value:'待料'},{text:'另约时间',value:'另约时间'},{text:'其他',value:'其他'}]"
                    required
                    title="维修状态"
                    v-model="maintain.status_name"/>
          <v-input :required="maintain.status_name !== '已完成'" title="维修备注" type="textarea" v-model="maintain.content"/>
    ...
    <section class="margin20" v-if="approver.length">
      ...
      <d-approval-process :accepts="info_data.lookUser.map(item=>item.username).join('，')" :approver="approver"/>
    </section>
    <!-- 审批流程结束 -->
    ...
    <v-picker :options="options.assignType"
              class="ml20 mr20"
              required
              text="type_name"
              title="请选择类型"
    ...
    <a @click="approvalRejectMantain" class="form-submit-block form-submit-block__reject" v-if="canIApproval">维修未通过</a>
    ...
    <a @click="rejectReport" class="form-submit-block form-submit-block__reject" v-if="canIReject">拒接任务</a>
...
<script>
  ...
  import {
    ...
    AssignRepairmen, CompletedMaintain,
    EngineerGroupAccept,
    GetEngineerDetail,
    GetEngineerGroup, GetRepairmenList, ApprovalRejectMantain,
    SendEngineerMaintainRecord, RejectEngineer, EngineerAddNote, GetProTypeList, ApprovalLeadRefuse
  ...
  const maintain_group = ['无班组', '强电班', '弱电班', '空调班', '电梯班', '综合班'];
  ...
  export default {
    ...
    computed: {
      ...
      canIAccept() {
        let role = [2, 3];
        return !!~role.indexOf(this.info_data.user_role.role) && this.info_data.status == 1;
      },
      // 是否可以拒绝工程单
      canIReject() {
      ...
      // 是否可以指派维修员
      canIAssign() {
        return (this.info_data.user_role.role == 2 && this.info_data.status == 2) && (this.info_data.group_type == this.info_data.user_role.type);
      },
      // 是否可以审批维修记录(通过或者不通过)
      canIApproval() {
        return this.info_data.user_role.role == 2 && this.info_data.status == 4 && this.info_data.pro_follow.slice(-1)[0].pro_follow.some(item => {
    ...
    methods: {
      ...
      addNote() {
        ...
        EngineerAddNote(this.$route.params.id, this.notes).then(res => {
      ...
      sendMaintain() {
        ...
        SendEngineerMaintainRecord(data).then(res => {
          loading.clear();
      ...
      acceptReport() {
        ...
        function startAccept(group) {
          EngineerGroupAccept(_this.$route.params.id, group).then(res => {
      ...
      rejectReport() {
        ...
        this.$createDialog({
          ...
          onConfirm: (event, value) => {
            ...
            RejectEngineer(this.$route.params.id, value).then(res => {
      ...
      assignRepairmen() {
        ...
        GetRepairmenList().then(res => {
          this.$createPicker({
            ...
            onSelect: (value, index, text) => {
              AssignRepairmen(value[0], this.$route.params.id, this.form_model.assignType).then(res => {
                this.$toast.success({message: '已成功指派维修员！', mask: true});
                setTimeout(() => {
                  location.reload();
                }, 1000);
              }).catch(err => {
      ...
      completedMaintain() {
        this.$createDialog({
          ...
          type: 'confirm',
          onConfirm: () => {
            CompletedMaintain(this.$route.params.id).then(res => {
              this.$toast.success({message: '已提交完成，请等待领班审核', mask: true});
              setTimeout(() => {
                location.reload();
              }, 1000);
            }).catch(err => {
      ...
      approvalAgreeMaintain() {
        this.$createDialog({
          ...
          type: "confirm",
          onConfirm: () => {
            let follow_id = this.info_data.pro_follow.slice(-1)[0].pro_follow.slice(-1)[0].id;
            ApprovalAgreeMaintain(this.$route.params.id, follow_id).then(res => {
      ...
      approvalRejectMantain() {
        ...
        async function onEdited(event, value) {
          ...
          let follow_id = _this.info_data.pro_follow.slice(-1)[0].pro_follow.slice(-1)[0].id;

          ApprovalRejectMantain(_this.$route.params.id, value, follow_id).then(res => {
      ...
      approvalDismissMantain() {
        ...
        this.$createDialog({
          ...
          onConfirm: (event, value) => {
            ...
            ApprovalLeadRefuse(_this.$route.params.id, value).then(res => {
              _this.$toast.success({message: '操作成功', mask: true});
              setTimeout(() => {
                location.reload();
              }, 1000)
