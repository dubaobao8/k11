<template>
  <div>
    ...
    <a @click="onFollowDone" class="form-submit-block" v-if="(info_data.status === 3 || info_data.status === 7) && !isPc()">跟进完成</a>
    ...
    methods: {
      ...
      agreeApproval() {
        this.ex.confirm('提示', '确定同意该清洁大做安排？').then(res => {
          request.post('/api/clean/AgreeCleanTask', {port_id: this.port_id}).then(res => {
      ...
      async onFollowDone() {
        ...
        try {
          await request.post('/api/clean/FinishFollow', {port_id: this.port_id});
