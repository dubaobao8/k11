<template>
  <div class="home-page bg-grey report">
    ...
    <main class="fs08">
      ...
      <!-- 是否为危机事件 -->
      <div @click="choosePick('is_danger')" class="form-inline">
        <div class="form-title fx100">
          是否为危机事件
          <span class="required">*</span>
        </div>
        <div class="crbbb dpfx fxac">
          <span v-if="!form.is_danger">请选择&nbsp;</span>
          <span v-else>{{form.is_danger | isDanger}}</span>
          <i class="ml5 fa fa-chevron-right"></i>
        </div>
      </div>
      ...
      <v-file-container
        :files="file_url"
        @del:file="delMxFile(file_url[$event].id)"
        auto_del
        auto_upload
        limit_title
      />
          ...
          <template v-for="(item,index) in form.look_group">
            <div :key="index" class="look-group__wrap">
              <div class="look-group__item">
                <span>{{item.name}}</span>
                <del @click="form.look_group.splice(index,1)" class="look-group__del">
                  <i class="del-icon"></i>
                </del>
              </div>
            </div>
    ...
    saveToDraft() {
      ...
      request
        ...
        .then(res => {
          ...
          request
            .post("/api/Draft/PortDetail", { port_id: this.form.port_id })
            .then(res => {
...
.draft-box {
  ...
  box-shadow: 0 2px 7px rgba(48, 129, 251, 0.8);
  position: fixed;
  bottom: 1.5rem;
  right: 1.5rem;
  z-index: 9;
