      ...
      <div class="owh">
        <ul class="menu-items">
          <template v-for="(item, index) in menuList">
            <a :class="{unopened:!item.open}" :key="index" @click="showSelection(item)" class="m-wrp" v-if="item.show">
  ...
  export default {
    ...
    methods: {
      ...
      async profolioEntry() {
        ...
        try {
          var access_id = (await request.post('/api/profolio/LookUser')).data.data;
        ...
        if (status == 2 || status == 3 || access_id.indexOf(user_id) !== -1) {
          ...
          await this.$router.push('/profolio_detail/' + localStorage.getItem('user_id'));
        }
