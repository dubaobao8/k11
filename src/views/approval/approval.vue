  ...
  <div class="position-relative">
    <main class="bg-white">
      ...
      <section :class="{'scroll-wrp-action':isPickerStatus}" class="scroll-wrp">
        <v-list-reports :extra-params="extraParams"
    ...
    <!-- 悬浮按钮 -->
    <div :class="{active:isPickerStatus}"
...
<script>
  ...
  import {AgreeMultipleIncidentAndPatrolReports, RejectMultipleIncidentAndPatrolReports} from "@/api/approval/approval";
  ...
  export default {
    ...
    methods: {
      ...
      changePickStatus() {
        if (this.isPickerStatus) {
          ...
          this.$toast({message: '已退出多选模式', position: 'bottom', duration: 1500});
        } else {
          this.isPickerStatus = true;
          this.$toast({message: '已进入多选模式', position: 'bottom', duration: 1500});
        }
      },
      // 搜索框选择完毕
      onSelect(data) {
      ...
      incidentMultiAgree() {
        var _this = this;
        this.showApprovalBox().then(res => {
          AgreeMultipleIncidentAndPatrolReports(res.content, res.star, this.pickedList).then(res => {
      ...
      incidentMultiReject() {
        var _this = this;
        this.showApprovalBox().then(res => {
          RejectMultipleIncidentAndPatrolReports(res.content, res.star, this.pickedList).then(res => {
            _this.$toast.success("操作成功！");
            setTimeout(() => {
              location.reload();
            }, 500);
      ...
      async showApprovalBox() {
        ...
        return await new Promise((resolve, reject) => {
          ...
          this.$createDialog({
            ...
            var title = h("p", {"class": {"mt20": true}, slot: "title"}, "请填写审批意见及评星");

            var content_main = [];
            content_main.push(h("cube-input", {
                "class": {
                  "ml20": true,
                  "mr20": true,
                  "mt15": true
    ...
    computed: {
      isShowMultiAction() {
        return !!multiActionMap[this.ReportType] && !!multiActionMap[this.ReportType].length
      },
      multiAction() {
