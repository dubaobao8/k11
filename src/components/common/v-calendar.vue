  ...
  <div class="calendar-wrp">
    ...
    <section class="calendar-table">
      <ul class="calendar-table-head">
        <li :key="week" v-for="week in days">
          {{week}}
        </li>
      </ul>

      <ol class="calendar-table-body">
        <li :class="setDayClass(day)"
        ...
        >
          ...
          <span :class="{'curdate':isSameDay(current,day.moment)}"
                class="selected"
                v-if="isSameDay(select,day.moment)"></span>
        </li>
      </ol>
    </section>
    <!-- 图例 -->
    <section v-if="false">
      ...
      <ul class="dpfx calendar-table-body mb20 fxjar">
        <div class="dpfx fxdc fxac mr15 ml15">
          <li class="curdate" style="margin:0">{{current.date()}}
            <span class="selected curdate"></span>
          </li>
          <p class="fs07 cr-grey mt10">今日</p>
        </div>
        <div class="dpfx fxdc fxac mr15 ml15">
          <li class="" style="margin:0">{{current.date()}}
    ...
    <section class="cr3" v-if="!modify">
      ...
      <div v-if="setting || (update  && CanBeUpdate())">
        <p class="ml5 mr5 mt15 fw-b fs1-04"
           style="color: #484952;"
           v-if="cur_calendar.tobelist&&cur_calendar.tobelist.length">当日待安排的任务</p>
        <ul>
          <li :key="task.task_id + select.valueOf()"
            ...
            <p>
              <span>当日次数：</span>
              <v-counter :max="getTaskCount(task.task_id) + task.count"
                         :value="task.count"
          ...
          <li :key="task.task_id + select.valueOf()"
            ...
            <div class="mt20 follow" v-if="task.plan_id && showFollowing && !isPc()">
              ...
              <div class="form-group">
                ...
                <v-file-container :files="task.follow_edit.files" :only-camera="true" auto_del auto_upload required/>
              </div>
              ...
              <div v-for="(item,index) in task.follow">
                ...
                <div v-if="item.curImg && canAddCommment">
                  <label class="add-commemt">
                    ...
                    <textarea class="add-commemt__field"
                              placeholder="请输入备注"
                              rows="2"
                              type="text"
                              v-model="item.curImgComment"/>
                  </label>
                  <a @click="onAddComment(item,item.curImgComment)"
                     class="form-submit-block"
                     style="margin: 15px 0;background-color:#2196f3;">提交当前附件备注</a>
                </div>
    ...
    <div v-if="cur_modify.assigned && modify">
      ...
      <ul class="modify-list" v-if="cur_modify.tobelist.length">
        <li class="modify-list-item form-group"
        ...
        >
          ...
          <div v-for="(item,index) in task.follow">
            <v-detail-title :title="'跟进记录' + (index + 1)" del-text="删除跟进" @del="onDelFollow(task.follow,item,index)"/>
            <v-input required title="跟进内容" v-model="item.content"/>
            <v-file-container style="border-bottom: none;" :files="item.files" :only-camera="true" required/>
    ...
    <!-- 提交清洁大做 -->
    <a @click="$emit('submit')" class="form-submit-block" style="margin: 30px 10px 0;" v-if="setting">
      提交清洁大做
    </a>
    <!-- 修改清洁大做 -->
    <a @click="$emit('submit')"
...
<script>
  ...
  export default {
    name: "v-calendar",
    components: {ActionLine, VDetailTitle, VFilePreview, DInfoItem, VFileContainer, VInput, VCounter},
    props: {
      tasklist: null,
      // 是否可以安排任务
      setting: {
    ...
    methods: {
      ...
      setDayClass(date) {
        return {
          ...
          'tobe-assign': date.tobelist.length,
          'assigined': date.assigned
        }
      },
      // 比较是否在日期范围内
      isWithinRange(startmoment, endmoment, moment) {
        return moment.isBetween(startmoment.clone().startOf('day').subtract(1, 'day'), endmoment.clone().startOf('day').add(1, 'day'));
      },
      // 是否同一天
      ...
      calcTaskCount(id) {
        ...
        let count_sum = taskItem.exec_date.map(exec => exec.count).reduce((total, count) => total + count, 0);
      ...
      onSetCount(calendar_task, count) {
        ...
        if (count > 0) {
          let execItem = task.exec_date.find(item => this.isSameDay(moment(item.date, 'YYYY/MM/DD'), this.select));
          if (execItem) {
          ...
          let execIndex = task.exec_date.findIndex(item => this.isSameDay(moment(item.date, 'YYYY/MM/DD'), this.select));
          task.exec_date.splice(execIndex, 1);
        }

        calendar_task.count = count;
        this.cur_calendar.assigned = this.cur_calendar.tobelist.some(item => item.count > 0);
      ...
      attachToDate() {
        this.dateList.forEach(day => {
          ...
          let calendarItem = this.calendar.find(item => this.isSameDay(item.moment, dayMoment));
          if (calendarItem) {
      ...
      submitFollow(task) {
        ...
        request.post('/api/clean/FollowUp', data).then(res => {
      ...
      onAddComment(item, comment) {
        ...
        request.post('/api/clean/AdminReason', data).then(res => {
      ...
      onDelFollow(follow, item, index) {
        ...
        request.post('/api/clean/DeleteFollow', {follow_id: item.id}).then(res => {
          this.$toast('删除成功！');
      ...
      onMofifySumit() {
        ...
        request.post('/api/clean/AgreeFollow', data).then(res => {
    ...
    computed: {
      isAssignToday() {
        return this.cur_calendar.tobelist && this.cur_calendar.tobelist.some(item => item.count > 0);
      }
    }
  }
</script>
...
<style scoped>
  ...
  .calendar-table-body li.no-selected-month.tobe-assign.assigined .calendar-day-text {
    color: #7ccaff;
  }
