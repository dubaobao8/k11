...
<template>
  <div v-show="files.length">
    <cube-popup :visible="videoStatus" @mask-click="clickMask">
    ...
    <section class="thumb-wrp">
      <swiper :options="config.thumb" ref="thumb" @slideChange="onThumbChange">
        <swiper-slide v-for="(item,index) in AllFiles.thumb_files">
          <span
            ...
            class="thumb__item"
          >
            <img :src="item.image" class="thumb__img" />
      ...
      <!-- 缩略图切换按钮-->
      <van-icon
        @click="thumb_swiper.slidePrev()"
        class="thumb__arrow thumb__left"
        name="arrow-left"
      />
      <van-icon @click="thumb_swiper.slideNext()" class="thumb__arrow thumb__right" name="arrow" />
    </section>
    ...
    <transition name="fade">
      <section class="viewer-wrp" v-show="isShow">
        ...
        <swiper :options="config.viewer" class="viewer-swiper" ref="viewer">
          <swiper-slide
            :key="Date.now() * Math.random()"
            v-for="(item,index) in AllFiles.viewer_files"
          >
            <div class="viewer__item swiper-zoom-container">
              <img :src="item.image" class="viewer__img" />
            </div>
          </swiper-slide>
        </swiper>
        ...
        <!-- 大图切换按钮-->
        <van-icon
          @click="viewer_swiper.slidePrev()"
          class="viewer__arrow viewer__left"
          name="arrow-left"
          v-if="isPc"
        />
        <van-icon
          @click="viewer_swiper.slideNext()"
...
export default {
    ...
    showViewer(item, event) {
      ...
      if (item.viewer_index !== null) {
        ...
        if (docReg.test(item.image) && isPc()) {
          ...
          defaultOpen = false;
          window.open(
            "https://view.officeapps.live.com/op/view.aspx?src=" + item.image
          );
        }
      }
  ...
  mounted() {
    ...
    this.viewer_swiper.on("slideChange", () => {
      ...
      this.currentTitle = this.AllFiles.viewer_files[
        this.currentViewerIndex
      ].title;
      this.currentRemark = this.AllFiles.viewer_files[
        this.currentViewerIndex
      ].remark;
      this.thumbGoto(
        this.AllFiles.viewer_files[this.currentViewerIndex].thumb_index
      );
    });
  }
};
