...
export default {
      ...
      if (!this.filterPicker) {
        this.filterPicker = this.$createCascadePicker({
          ...
          onSelect: (value, index, text) => {
            this.selectedText = text[0] + " " + text[1];
  ...
  watch: {
    ...
    newSearchParam: {
      handler(newValue, oldValue) {
        if (newValue.text) {
          this.filterReportType = newValue.type;
          this.filterReportStatus = newValue.status;
          this.selectedText = newValue.selectedText;
          this.filterValue = newValue.text;
        } else {
          this.selectedText = newValue.selectedText;
          this.$emit("select", {
            text: newValue.text,
  ...
  mounted() {
    GetSearchFilter().then(res => {
      this.$_selection = res.data.data.map(item => {
        return {
