    ...
    <div @click="onToDetail">
      ...
      <template v-if="listType === 'normal'">
        <div
          ...
          v-for="item in list"
        >
          <report-list-item-new :data="item" :key="Date.now() * Math.random()" />
        </div>
      </template>
      <!-- 草稿箱列表 -->
...
import ReportListItemNew from "@/components/report-list-item/newReportList/report-list-item-new.vue";
import ReportListItemDraft from "@/components/report-list-item/report-list-item-draft";

export default {
  name: "v-list-reports",
  components: { ReportListItemDraft, ReportListItemNew, VList },
  props: {
    url: {
      type: String
    },
